 ...
 * Shadow Dev Page UI
 *
 * Purpose: Renders development-only controls for shadow catalog setup.
...
import type { ShadowDevActionResult } from '@/shadow/views/dev/lib/get-dev-action-result';
import { authenticateShadowDevUser } from '@/shadow/views/dev/server/authenticate-shadow-dev-user';
import { revalidateShadowPagesAction } from '@/shadow/views/dev/server/revalidate-shadow-pages';
import { seedShadowCatalog } from '@/shadow/views/dev/server/seed-shadow-catalog';

type ShadowDevPageProps = {
  actionResult: ShadowDevActionResult | null;
  dictionary: ShadowDictionary;
};

/**
 * Renders the complete shadow dev tooling page.
 *
 * @param props - Localized dictionary copy and latest action result.
 * @returns A server-rendered dev control surface.
 */
export function ShadowDevPage(props: ShadowDevPageProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-8 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section aria-label={dictionary.dev.actionsLabel} className="grid gap-4 md:grid-cols-3">
        <form action={authenticateShadowDevUser} className="rounded-lg border bg-white p-5 shadow-sm">
        ...
        <form action={seedShadowCatalog} className="rounded-lg border bg-white p-5 shadow-sm">
        ...
        <form action={revalidateShadowPagesAction} className="rounded-lg border bg-white p-5 shadow-sm">
