...
/**
 * Generate cart context for the AI assistant
 * @param cart - Cart state object
 * @returns Cart context string
 */
export function getCartContext(cart: any): string {
...
- Cart Items: The user currently has the following items in their shopping cart:
${JSON.stringify(cart.items.map((item: any) => ({
  ...
  total: item.total,
  color: item.color,
  product: item.product, // Full product details if available
})), null, 2)}
- Cart Total: ${cart.total || 0}
- Number of Items: ${cart.items.length}

Use this information to provide recommendations, comparisons, or suggestions based on items in the user's cart.`;
}