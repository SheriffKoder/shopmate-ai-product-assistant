...
import { handleApiError, ShopMateError } from '@/features/ai-assistant/lib/errors';
...
export async function DELETE(
  ...
  try {
    ...
    if (!chat) {
      ...
      return new ShopMateError('not_found:api', 'Chat not found').toResponse();
    }

    // Verify chat belongs to user
    if (chat.userId !== user.id) {
      logger.warn(`[DELETE /api/chat/[chatId]] Unauthorized: Chat ${chatId} does not belong to user ${user.id}`);
      return new ShopMateError('forbidden:api', 'You do not have permission to delete this chat').toResponse();
    }
